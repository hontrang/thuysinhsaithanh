            ...
            <div class="ibox-content">
                <form method="POST" action="" class="form-horizontal">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-md-8">
                            <input type="text" name="position" value="<?php echo set_value('position',$info->position)?>" class="form-control">
                        ...
                        <div class="col-sm-4">
                           <select class="form-control" name="department_id">
                                ...
                                <?php if(isset($cats))foreach ($cats as $cat){
                                    $select = false;
                                    if($info->department_id==$cat->id){$select=true;};
                                    ?>
                                    <option value="<?php echo $cat->id;?>" <?php echo set_select("department_id", $cat->id,$select);?>>---- <?php echo $cat->name;?></option>
                        ...
                        <div class="col-sm-4">
                            <select class="form-control" name="room_id">
                                ...
                                <?php if(isset($rooms))foreach ($rooms as $room){
                                    $select = false;
                                    if($info->room_id==$room->id){$select=true;};
                                    ?>
                                    <option value="<?php echo $room->id;?>" <?php echo set_select("room_id", $room->id, $select);?>>--- <?php echo $room->name;?></option>
                            ...
                            <table class="table table-bordered">
                                ...
                                <tr>
                                    <?php for($i=2; $i<9;$i++){
                                        ...
                                        $check = '';
                                        if(isset($schedule_list) and $schedule_list['am'][$i] == 1)
                                            $check = 'checked="checked"';
                                        ?>
                                        <td>
                                            <div><label class="checkbox-inline i-checks"> <input type="checkbox" name="time[am][]" value="<?php echo $i;?>" <?php echo $check;?>> Sáng</label></div>
                                        </td>
                                    <?php } ?>
                                </tr>
                                <tr>
                                    <?php for($i=2; $i<9;$i++){
                                        $check = '';
                                        if(isset($schedule_list) and $schedule_list['pm'][$i] == 1)
                                            $check = 'checked="checked"';
                                        ?>
                                        <td>
                                            <div><label class="checkbox-inline i-checks"> <input type="checkbox" name="time[pm][]" value="<?php echo $i;?>" <?php echo $check;?>> Chiều</label></div>
                                        </td>
                                    <?php } ?>
                                </tr>
