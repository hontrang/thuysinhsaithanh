$(document).ready(function() {
	...
	var saveEditedImage = function(item) {
		...
		// send request
		if (item.appended) {
			$.post('/admin/construction/editimage', {_file: item.file, _editor: JSON.stringify(item.editor), fileuploader: 1}, function() {
	...
	$('input[name="files"]').fileuploader({
		...
		thumbnails: {
			box: '<div class="fileuploader-items" >' +
                      '<ul class="fileuploader-items-list ">' +
					      '<li class="fileuploader-thumbnails-input col-xs-6 col-md-6 col-sm-3 col-lg-3"><div class="fileuploader-thumbnails-input-inner " style="min-width: 100px;"><span style="font-size: 30px;"><i class="fa fa-picture-o"></i></span><br /><span style="font-size: 15px; font-weight:bold">Click here<br />to add an image</span></div></li>' +
                      '</ul>' +
                  '</div>',
		...
		onRemove: function(item) {
			$.post('/admin/construction/delimage', {
				file: item.name
			});
		},
		editor: {
