$(document).ready(function() {
	...
	var saveEditedImage = function(item) {
		...
		// send request
		if (item.appended) {
			$.post('/khach-hang/tu-van/editimage', {_file: item.file, _editor: JSON.stringify(item.editor), fileuploader: 1}, function() {
	...
	$('input[name="files"]').fileuploader({
		...
		onRemove: function(item) {
			$.post('/khach-hang/tu-van/delimage', {
				file: item.name
			});
		},
		editor: {
