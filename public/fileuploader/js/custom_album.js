$(document).ready(function() {
	...
	var saveEditedImage = function(item) {
		...
		// send request
		if (item.appended) {
			$.post('/admin/album/editimage', {_file: item.file, _editor: JSON.stringify(item.editor), fileuploader: 1}, function() {
		...
		thumbnails: {
			box: '<div class="fileuploader-items" >' +
                      '<ul class="fileuploader-items-list ">' +
					      '<li class="fileuploader-thumbnails-input col-xs-6 col-md-6 col-sm-3 col-lg-3"><div class="fileuploader-thumbnails-input-inner " style="min-width: 100px;"><span style="font-size: 30px;"><i class="fa fa-picture-o"></i></span><br /><span style="font-size: 15px; font-weight:bold">Nhấn vào đây<br />để thêm hình ảnh</span></div></li>' +
                      '</ul>' +
                  '</div>',
			item: '<li class="fileuploader-item col-xs-6  col-md-6 col-sm-3 col-lg-3">' +
                       ...
                       '</div>' +
            			'<div style="text-align: center;margin-top: 3px;"><input type="input" id="value_${data.id}" class="file_title" data-file="${name}" data-id="${data.id}" value="${title}"><button data-file="${name}" data-id="${data.id}" class="save_btn" id="save_${data.id}"><i class="fa fa-check"></i> Đã lưu</button></div>' +
                   '</li>',
			item2: '<li class="fileuploader-item col-xs-6 col-md-6 col-sm-3 col-lg-3">' +
				       '<div class="fileuploader-item-inner">' +
                           '<div class="thumbnail-holder">${image}</div>' +
                           '<div class="actions-holder">' +
                               '<a class="fileuploader-action fileuploader-action-remove" title="${captions.remove}"><i class="remove"></i></a>' +
                           ...
                           '</div>' +
                       '</div>' +
            			'<div style="text-align: center;margin-top: 3px;"><input id="value_${data.id}" type="input" class="file_title" data-file="${name}"  data-id="${data.id}" value="${data.title}"><button data-file="${name}" data-id="${data.id}" class="save_btn" id="save_${data.id}"><i class="fa fa-check"></i> Đã lưu</button></div>' +
                   '</li>',
